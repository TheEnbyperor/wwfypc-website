...
class wpdb {
		...
		if ( $this->has_cap( 'collation' ) && ! empty( $charset ) ) {
			...
			if ( $this->use_mysqli ) {
				if ( function_exists( 'mysql_set_charset' ) && $this->has_cap( 'set_charset' ) ) {
					$set_charset_succeeded = mysql_set_charset( $dbh, $charset );
				}
	...
	public function set_sql_mode( $modes = array() ) {
		if ( empty( $modes ) ) {
			if ( $this->use_mysqli ) {
				$res = mysql_query( $this->dbh, 'SELECT @@SESSION.sql_mode' );
			} else {
			...
			if ( $this->use_mysqli ) {
				$modes_array = mysql_fetch_array( $res );
		...
		if ( $this->use_mysqli ) {
			mysql_query( $this->dbh, "SET SESSION sql_mode='$modes_str'" );
		} else {
	...
	public function select( $db, $dbh = null ) {
		...
		if ( $this->use_mysqli ) {
			$success = mysql_select_db( $dbh, $db );
		} else {
	...
	/**
	 * Real escape, using mysql_real_escape_string() or mysql_real_escape_string()
	 *
	...
	function _real_escape( $string ) {
		if ( $this->dbh ) {
			if ( $this->use_mysqli ) {
				$escaped = mysql_real_escape_string( $this->dbh, $string );
			} else {
	...
	public function flush() {
		...
		if ( $this->use_mysqli && $this->result instanceof mysql_result ) {
			...
			while ( mysql_more_results( $this->dbh ) ) {
				mysql_next_result( $this->dbh );
			}
	...
	public function db_connect( $allow_bail = true ) {
		 ...
		 * $new_link parameter exists for mysql_* functions.
		 */
		...
		if ( $this->use_mysqli ) {
			...
			// mysql_real_connect doesn't support the host param including a port or socket
			...
			if ( WP_DEBUG ) {
				mysql_real_connect( $this->dbh, $host, $this->dbuser, $this->dbpassword, null, $port, $socket, $client_flags );
			} else {
				@mysql_real_connect( $this->dbh, $host, $this->dbuser, $this->dbpassword, null, $port, $socket, $client_flags );
			}
			...
			foreach ( $data as $col => $value ) {
				if ( ! empty( $value['db'] ) ) {
					...
					if ( $this->charset ) {
						...
						if ( $this->use_mysqli ) {
							$connection_charset = mysql_character_set_name( $this->dbh );
						} else {
