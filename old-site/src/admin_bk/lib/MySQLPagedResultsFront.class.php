...
class MySQLPagedResults extends ConnectionMgr
{
    ...
    function getPrevNav()
    {
        ...
        if (!$this->isFirstPage())
        {
            $nav .= "<a href=\"javascript: submitPaging('frmPaging',$this->current_page-1);\" >" . $this->previous_link_text . '</a>';
        } else
        {
    ...
    function getNextNav()
    {
        ...
        if (!$this->isLastPage())
        {
            $nav .= "<a href=\"javascript: submitPaging('frmPaging',$this->current_page+1);\" >" . $this->next_link_text . '</a>';
        } else
        {
    ...
    function getFirstNav()
    {
        ...
        if (!$this->isFirstPage())
        {
            $nav .= "<div class='arrows' style='margin-right:10px;'><a href=\"javascript: submitPaging('frmPaging',1)\" >" . $this->first_link_text . '</a></div>';
        } else
        {
            $nav .=  "<div class='arrows' style='margin-right:10px;'>".$this->first_link_text."</div>";
        }
        return $nav;
    }
    ...
    function getLastNav()
    {
        ...
        if (!$this->isLastPage())
        {
            $nav .= "<div class='arrows' style='margin-left:10px;'><a href=\"javascript: submitPaging('frmPaging'," . $this->totalPages() . ");\" >" . $this->last_link_text . '</a></div>';
        } else
        {
            $nav .= "<div class='arrows' style='margin-left:10px;'>".$this->last_link_text."</div>";
        }
    ...
    function getPagesNav()
    {
        ...
        for ($i = $this->getStartNumber(); $i < $this->getStartNumber() + $this->getEndNumber(); $i++)
        {
            ...
            {
                $nav .= "<div class='pagingtxt'><a href=\"javascript: submitPaging('frmPaging',$i)\" >$i</a></div>";
            } else
            {
                $nav .= "<div class='pagingtxtS'>$i</div>";
            }
