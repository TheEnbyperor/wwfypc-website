<?php
    ...
    if(isset($_REQUEST['DetailMode']) && $_REQUEST['DetailMode'] == "Add") {
        ...
        if(isset($_POST['id']) && $_POST['id'] != '') {
            ...
            $update             = update('tbl_unlock_network', $data, "id = $id");
        } else {
            $displayMessage     = ADDMESSAGE;
            $data['created_on'] = date('Y-m-d H:m:s');
            $save = save('tbl_unlock_network', $data);
        }
    ...
    if(isset($_REQUEST['DetailMode'])&& $_REQUEST['DetailMode'] == "Edit") {
        $id = $_REQUEST['id'];
        $value = read('one', 'tbl_unlock_network', "id = $id", 'id DESC');
    }

    if(isset($_REQUEST['DetailMode'])&& $_REQUEST['DetailMode'] == "Delete") {
        $id                 = $_REQUEST['id'];
        $displayMessage     = DELETEMESSAGE;
        $delete             = delete('tbl_unlock_network', "id = $id");

        header("Location: control-panel.php?menuId=$menuId&module=$module&fname=$fname&message=$displayMessage");
    }

    $data = read('all', 'tbl_unlock_network', 'status = 1', 'id DESC');
?>
    ...
    <tbody>
        ...
        <tr>
            <td colspan="4" class="cptxt" valign="top" align="center">
                <form name="frmUserDetail" id="frmUserDetail" action="control-panel.php?menuId=unlock&module=Unlock&fname=manage-network.php&DetailMode=Add" method="post" enctype="multipart/form-data">
                    <table width="95%" border="0" cellspacing="0" cellpadding="2">
                        <input name="module" id="module" type="hidden" value="Unlock">
                        <input name="fname" id="fname" type="hidden" value="manage-network.php">

                        <tbody>
                            <tr>
                                <td colspan="4" align="Left" class="tblHeading" height="25">ADD network</td>
                            ...
                            <tr>
                                <td class="cptxt">&nbsp;</td>
                                <td align="left" valign="top" class="cptxt">Network Name:</td>
        ...
        <tr>
            <td colspan="4" class="cptxt" valign="top" align="center">
                <table width="95%" border="0" cellspacing="0" cellpadding="2" style="border-width:2px; border-style:solid; border-color:#F7F7F7;">
                    <tbody>
                        <tr class="tblHeading">
                            <td width="84%" height="25" align="left"><strong>Network Name</strong></td>
                        ...
                        <?php foreach ($data as $key => $v) { ?>
                            <tr bgcolor="#f6f6f6">
                                ...
                                <td height="25" align="center" valign="top" class="cptxt">
                                    <a href="control-panel.php?menuId=unlock&id=<?php echo $v->id; ?>&module=Unlock&fname=manage-network.php&DetailMode=Edit" title="Edit Record"><img src="images/edit.gif" border="0"></a>
                                </td>

                                <td align="center" valign="top" class="cptxt">
                                    <a href="control-panel.php?menuId=unlock&id=<?php echo $v->id; ?>&module=Unlock&fname=manage-network.php&DetailMode=Delete" title="Delete Record" onclick="javascript: return deleteRecordsChk();"><img src="images/delete.gif" border="0"></a>
                                </td>
                            </tr>
                        <?php } ?>
                    </tbody>
